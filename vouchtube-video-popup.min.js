let is_open=false;function closeVideoPopup(){document.getElementById("video-widget").style.display="none";is_open=false}function openVideoPopup(){if(is_open){closeVideoPopup();is_open=false}else{document.getElementById("video-widget").style.display="flex";is_open=true}}const page_head=document.head;const body_html=document.body;function elementFromHtml(html){const template=document.createElement("template");html=html.trim();template.innerHTML=html;return template.content.firstChild}const html_fonts=elementFromHtml(`
  <link
  rel="stylesheet"
...
`);page_head.append(html_fonts,html_icons,html_css);body_html.append(html_widget_icon,html_popup_container);
